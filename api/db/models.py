...
class Telefone_Funcionario(SQLModel, table=True):
    codigo_func: int = Field(default=None, foreign_key="funcionario.codigo", primary_key=True)
...
class Funcionario(SQLModel, table=True):
    cnpj_empresa: str | None = Field(default=None, nullable=False, foreign_key='empresa.cnpj')
    codigo: int | None = Field(sa_column=Column(BigInteger, Identity(always=True), default=None, primary_key=True))
    nome: str
    sexo: str | None = None
    cargo: Cargo
    ...
    telefones: list[Telefone_Funcionario] = Relationship(back_populates="funcionario")
...
class Motorista(SQLModel, table=True):
    ...
    codigo_funcionario: int = Field(unique=True, foreign_key="funcionario.codigo", ondelete="CASCADE")
...
class Modelo(SQLModel, table=True):
    codigo: int = Field(primary_key=True)
    codigo_categoria: int | None = Field(default=None, nullable=False, foreign_key="categoria.codigo")
    nome: str = Field(unique=True)

class Veiculo(SQLModel, table=True):
    placa: str = Field(primary_key=True, max_length=7)
    cnpj_empresa: str | None = Field(nullable=False, foreign_key="empresa.cnpj")
    codigo_modelo: int | None = Field(nullable=False, foreign_key="modelo.codigo")
    km: float | None = None
...
class Manutencao(SQLModel, table=True):
    id: int | None = Field(sa_column=Column(BigInteger, Identity(always=True), default=None, primary_key=True))
    placa_veiculo: str = Field(foreign_key="veiculo.placa", sa_column_kwargs={"onupdate": "CASCADE"})
    data_hora: datetime
...
class Manutencao_Servico(SQLModel, table=True):
    id_manutencao: int | None = Field(default=None, foreign_key="manutencao.id", primary_key=True)
    codigo_servico: int | None = Field(default=None, foreign_key="servico.codigo", primary_key=True)

class Ocorrencia(SQLModel, table=True):
    id: int | None = Field(sa_column=Column(Integer, Identity(always=True), primary_key=True, default=None))
...
class Acidente(SQLModel, table=True):
    id_ocorrencia: int | None = Field(foreign_key="ocorrencia.id", primary_key=True)
    descricao: str | None = None
    gravidade: Grau
...
class Itinerario(SQLModel, table=True):
    codigo_linha: str = Field(foreign_key="linha.codigo", ondelete="CASCADE", sa_column_kwargs={"onupdate": "CASCADE"}, primary_key=True)
...
class Viagem(SQLModel, table=True):
    ...
    codigo_cobrador: int = Field(primary_key=True, foreign_key="funcionario.codigo")
    codigo_motorista: int = Field(primary_key=True, foreign_key="funcionario.codigo")
